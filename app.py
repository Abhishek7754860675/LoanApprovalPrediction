...
# =========================================================================
#  CUSTOM CSS  (with mobile-responsive tweaks)
# =========================================================================
st.markdown(
    """
    <style>
    ...
    @media (max-width: 640px) {
        ...
        .header {padding: 1.2rem 1.3rem; border-radius: 16px;}
        ...
        div.stButton > button, div.stDownloadButton > button {width: 100%; padding: 0.7rem 1rem;}
    }
    </style>
    """,
...
def build_confetti_burst_html(count=140):
    """Full-screen confetti burst (like st.balloons()) — no external libraries.
    Injects into the parent document so it covers the whole viewport, not just
    the small component box. Falls back to a locally-contained burst if the
    parent document isn't reachable for any reason."""
    return f"""
    <script>
      (function() {{
        ...
        function spawnInto(doc, win, fixedFull) {{
          ...
          for (let i = 0; i < count; i++) {{
            ...
            el.style.animation = `confetti-fall-full ${{dur}}ms cubic-bezier(0.15,0.7,0.4,1) ${{delay}}ms forwards`;
            wrap.appendChild(el);
          }}
          setTimeout(() => {{ wrap.remove(); styleEl.remove(); }}, 3200);
        }}
...
with st.sidebar:
    ...
    if st.session_state.history:
        ...
        st.download_button("Download history", data=csv, file_name="loan_prediction_history.csv", mime="text/csv", use_container_width=True)
...
# =========================================================================
    ...
    with top_left:
        components.html(build_animated_gauge_html(rd['score']), height=290)
    with top_right:
