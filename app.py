...
def _find_artifact(filename):
    """Look for a model/scaler file in a few common locations so this
    works whether 'Model/' sits next to app.py, one level up, or app.py
    itself lives inside a subfolder (local vs GitHub/Streamlit Cloud
    layouts often differ)."""
    candidates = [
    ...
    for path in candidates:
        if os.path.exists(path):
            return path
    # Nothing found - return the most likely path anyway so the
    # resulting error message shows a sensible location.
    return candidates[0]


MODEL_PATH = _find_artifact("loan_model.pkl")
...
def load_artifacts():
    if not os.path.exists(MODEL_PATH) or not os.path.exists(SCALER_PATH):
        st.error(
            "Model files not found. Make sure a 'Model' folder containing "
            "'loan_model.pkl' and 'scaler.pkl' is included in your repo, "
            "next to app.py (or one level above it).\n\n"
            f"Looked for:\n- {MODEL_PATH}\n- {SCALER_PATH}"
        )
        st.stop()
